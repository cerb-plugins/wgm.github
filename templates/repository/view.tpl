{$view_context = Context_GitHubRepository::ID}
	...
	<tbody style="cursor:pointer;">
		<tr class="{$tableRowClass}">
			<td align="center" rowspan="2" nowrap="nowrap" style="padding:5px;">
				{include file="devblocks:cerberusweb.core::internal/watchers/context_follow_button.tpl" context=$view_context context_id=$result.g_id}
			</td>
			<td colspan="{$smarty.foreach.headers.total}">
				<input type="checkbox" name="row_id[]" value="{$result.g_id}" style="display:none;">
				<a href="{devblocks_url}c=profiles&type=github_repository&id={$result.g_id}{/devblocks_url}" class="subject">{$result.g_owner_github_name}/{$result.g_name}</a>
				<button type="button" class="peek" onclick="genericAjaxPopup('peek','c=internal&a=showEntryPopup&id={$result.g_id}&view_id={$view->id}',null,false,'500');"><span class="glyphicons glyphicons-new-window-alt"></span></button>
			</td>
		</tr>
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column=="g_created_at" || $column=="g_pushed_at" || $column=="g_updated_at" || $column=="g_synced_at"}
			...
			{elseif $column=="g_url"}
				<td><a href="{$result.$column}" target="_blank">{$result.$column}</a></td>
			{else}
				<td>{$result.$column}</td>
			{/if}
		{/foreach}
...
<div style="padding-top:5px;">
	...
	<div style="float:left;" id="{$view->id}_actions">
		{*
		<button type="button" class="action-always-show action-explore" onclick="this.form.explore_from.value=$(this).closest('form').find('tbody input:checkbox:checked:first').val();this.form.a.value='viewExplore';this.form.submit();"><span class="glyphicons glyphicons-play-button"></span> {'common.explore'|devblocks_translate|lower}</button>
