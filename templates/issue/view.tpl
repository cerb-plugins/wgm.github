{$view_context = Context_GitHubIssue::ID}
	...
	<tbody style="cursor:pointer;">
		<tr class="{$tableRowClass}">
			...
			<td colspan="{$smarty.foreach.headers.total}">
				...
				<a href="{devblocks_url}c=profiles&type=example_object&id={$result.g_id}{/devblocks_url}" class="subject">{$result.g_title}</a>
		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column=="g_created_at" || $column=="g_updated_at" || $column=="g_closed_at" || $column=="g_synced_at"}
				<td title="{$result.$column|devblocks_date}">
					{if !empty($result.$column)}
						{$result.$column|devblocks_prettytime}&nbsp;
					{/if}
				</td>
			{elseif $column=="g_github_repository_id"}
				<td>
					{$repository = $repositories.{$result.$column}}
					{$repository->name}
				</td>
			{else}
				<td>{$result.$column}</td>
